...
for(x in 1:100){
  ...
  for(i in unique(df$year)){
    df$p_exposure[df$year==i]<-logit(betas_gammas3$g0_for_E[betas_gammas3$year==i], 
                                     betas_gammas3$g1_for_E[betas_gammas3$year==i], 
  ...
  for(i in unique(df$year)){
    df$p_disease[df$year==i]<-logit(betas_gammas3$b0_for_D[betas_gammas3$year==i], 
                                    betas_gammas3$b1_for_D[betas_gammas3$year==i], 
                                    df$confounder[df$year==i])
  }
  ## Simulating D
...
#ggsave("scenario3_fig1.jpg", width=9 ,height=6, units= "in", dpi=1200)
...
### Extracting the coefficient OR D | E, C
for(i in 1:100){ 
  d_e_c_or<-exp(list_select_coeff(listd, i))
}

### Extracting the 95%CI OR D | E, C
...
#ggsave("scenario3_fig2_vertical.jpg", width=9 ,height=6, units= "in", dpi=1200)
