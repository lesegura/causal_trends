...
pander(betas_gammas, caption = "Parameters that we are going to use for the simulation")
...
for(x in 1:100){
  ...
  for(i in unique(df$year)){
    df$p_exposure[df$year==i]<-logit(betas_gammas$g0_for_E[betas_gammas$year==i], 
                                     betas_gammas$g1_for_E[betas_gammas$year==i], 
                                     df$confounder[df$year==i])
  }
  ### Simulating E
  df$exposure<-runif(nrow(df),min=0, max= 1)< df$p_exposure
  df$exposure<-ifelse(df$exposure==TRUE, 1, 0)
  ### Estimating Probability of D
  for(i in unique(df$year)){
    df$p_disease[df$year==i]<-logit(betas_gammas$b0_for_D[betas_gammas$year==i], 
                                    betas_gammas$b1_for_D[betas_gammas$year==i], 
                                    df$confounder[df$year==i])
    }
  ## Simulating D
  df$disease<-runif(nrow(df),min=0, max= 1)< df$p_disease
  df$disease<-ifelse(df$disease==TRUE, 1, 0)
...
ggplot(data = df.plot, 
  ...
  scale_y_continuous(breaks = seq(0.00, 0.40, 0.05)) + 
  expand_limits(y=c(0.00, 0.40)) #+
...
#ggsave("scenario1_fig1.jpg", width=9 ,height=6, units= "in", dpi=1200)
...
#----------------------------------------------------------#
#                                                          #
#     Plotting the effects of C on E & D, and E on D       #
#                                                          #
#----------------------------------------------------------#


#df.plot2<-df.all.coef[c(10:1), ]


# library(forestplot, quietly = T)
# 
# graph.names<-matrix(c("2018", "2017", "2016", "2015", "2014", 
#                       "2013", "2012", "2011", "2010", "2009"), 
#                     byrow =T) ##creating a vector with the row names.
# 
# png("scenario1_figure_e_c.jpg", width = 6, height = 4, units = 'in', res = 1200) 
# forestplot(graph.names, c(df.plot2[, 2]) , 
#            c(df.plot2[, 3]), 
#            c(df.plot2[ ,4]), lwd.zero = 5,cex=5,
#            zero=1, cex=2, lineheight = "auto", new_page = TRUE, ci.vertices=T,
#            xlab ="Odds Ratio", 
#            title= " ", boxsize = 0.3, xticks=seq(0.00, 30.00, 2),
#            col=fpColors(box=c("darkblue"),
#                         line=c("darkblue")))
# dev.off()
# 
# png("scenario1_figure_d_c.jpg", width = 6, height = 4, units = 'in', res = 1200) 
# forestplot(graph.names, c(df.plot2[, 6]) , 
#            c(df.plot2[, 7]), 
#            c(df.plot2[ ,8]), lwd.zero = 5,cex=5,
#            zero=1, cex=2, lineheight = "auto", new_page = TRUE, ci.vertices=T,
#            xlab ="Odds Ratio", 
#            title= " ", boxsize = 0.3, xticks=seq(0.00, 30.00, 2),
#            col=fpColors(box=c("darkblue"),
...
# dev.off()
# 
# png("scenario1_figure_d_e.jpg", width = 6, height = 4, units = 'in', res = 1200) 
# forestplot(graph.names, c(df.plot2[, 10]) , 
#            c(df.plot2[ , 11]), 
#            c(df.plot2[ , 12]), lwd.zero = 5,cex=5,
#            zero=1, cex=2, lineheight = "auto", new_page = TRUE, ci.vertices=T,
#            xlab ="Odds Ratio", 
#            title= " ", boxsize = 0.3, xticks=seq(0.00, 30.00, 2),
#            col=fpColors(box=c("darkblue"),
...
#ggsave("scenario1_fig2_horizontal.jpg", width=9 ,height=6, units= "in", dpi=1200)

scenario1.or<-df.all.coef
